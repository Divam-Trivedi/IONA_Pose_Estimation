...
class FoundationPose:
  ...
  def register(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
    ...
    # print(depth.dtype, depth.shape, depth.max(), depth.min())
    ...
    if self.debug>=2:
      ...
      # print("rgb valid is ", rgb[valid])
      # print("xyz map  is ", xyz_map, "sum is", xyz_map[valid].sum())
    ...
    xyz_map = depth2xyzmap(depth, K)
    poses, vis = self.refiner.predict(mesh=self.mesh, mesh_tensors=self.mesh_tensors, rgb=rgb, depth=depth, K=K, ob_in_cams=poses.data.cpu().float(), normal_map=normal_map, xyz_map=xyz_map, glctx=self.glctx, mesh_diameter=self.diameter, iteration=iteration, get_vis=self.debug>=2)
    if vis is not None:
